...
sub new{
    my $type = shift;
    my %parm = @_;
    my $ref_seg = $parm{ref_seg};
    my $gtf_lineinfo = $parm{gtf_lineinfo};
...
# load the gtf info to the GTF_geneOB object, such as transcript, exon, CDS, codon, ...
sub load_gtf_info{
    my $gene = shift;
    my %parm = @_;
    my $gtf_lineinfo = $parm{gtf_lineinfo};
    ...
    # other info
    my ($gtf_source_or_transBiotype,$region_type,$st,$ed,$strand) = (split /\t+/, $gtf_lineinfo)[1,2,3,4,6];
...
sub add_cytoband_info{
    ...
    $gene->{cytoband} = get_cytoband(cytoBand_Href => $cytoBand_Href, refseg => $refseg, regionStP => $gene->{small_edge}, regionEdP => $gene->{large_edge});
    for my $_trans_ENSid (keys %{$gene->{trans_info}}){
        my $trans = $gene->{trans_info}->{$_trans_ENSid};
        $trans->{cytoband} = get_cytoband(cytoBand_Href => $cytoBand_Href, refseg => $refseg, regionStP => $trans->{small_edge}, regionEdP => $trans->{large_edge});
    }
}
