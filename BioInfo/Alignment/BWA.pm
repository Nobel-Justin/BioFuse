...
sub PEfqToSortBam{
    ...
    my %parm = @_;
    my $ref = $parm{ref};
    my $fqPathAf = $parm{fqPathAf};
    my $bamPath = $parm{bamPath};
    my $bwa = $parm{bwa};
    ...
    if($func eq 'mem'){
        ...
        $cmd =  "($bwa aln -L -m 800000 -l 27 -i 10 -k 3 -t 8 -e 10 -f $sai_1 $ref $fqPathAf->[0] 2>/dev/null) && "
               ."($bwa aln -L -m 800000 -l 27 -i 10 -k 3 -t 8 -e 10 -f $sai_2 $ref $fqPathAf->[1] 2>/dev/null) && "
               ."($bwa sampe -a $maxIns $RG_para $ref $sai_1 $sai_2 $fqPathAf->[0] $fqPathAf->[1] 2>/dev/null "
               .($discBuPair ? "| $awk " : '')
               ."| $samtools view -b -S $view_para -T $ref -o $raw_bamPath - 2>/dev/null) && "
               ."(rm -f $sai_1 $sai_2)";
    }
