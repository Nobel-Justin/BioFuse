...
use BioFuse::BioInfo::Depth::MixToPureT qw/ get_ObjCountOfPureTumorCell /;
...
$MODULE_NAME = 'BioFuse::BioInfo::Depth';
#----- version --------
$VERSION = "0.05";
$DATE = '2018-12-03';
...
sub deal_window_depth_info{
        ...
        if( $HasT_mode && $depth != 0 ){
            if( $role eq 'host' ){ # such as human genome
                if( $tissue eq 'ctrl' ){
                    ...
                    $depth = get_ObjCountOfPureTumorCell( ObjCount => $depth,
    ...
    if(    scalar(@pos_depth) == 0
        ...
        $this_winDepth_Href->{mean_adjdepth} = get_value_mean(value_Aref => \@pos_depth, Eng_SD_Ntimes => ($Eng_3SD ? 3 : 0), TrimRatio => $TmmRatio);
    }
}
...
sub get_givenWinItvalMeanDepth{
    ...
    if( scalar(@winDepth_pool) == 0 ){
        ...
        return get_value_mean(value_Aref => \@winDepth_pool, Eng_SD_Ntimes => 3);
    }
}
