...
use BioFuse::Visual::Windows qw/ get_pos_windowNO windowNO_to_posInterval /;
use BioFuse::Visual::ArrangeObj qw/ test_overlap_at_one_layer /;
require Exporter;
...
sub show_Y_axis{
    ...
    if( !defined $svg_obj ){
        $svg_obj = initialize_SVG_obj( bg_width => $axisZX + $xAxisLen + $bi_extWidth,
    ...
    while( $yLabValue <= $yMaxValue ){
        ...
        # short Label line
        $svg_obj->line( x1 => $axisZX + $rgtvtXshift + (1 + $yLabelLlen) * $rgtvtRatio,
                        y1 => $y_loc,
                        x2 => $axisZX + $rgtvtXshift + 1 * $rgtvtRatio,
                        y2 => $y_loc,
                        stroke => $yStrokeCol,
    ...
    if( defined $yLegd_Href){
        ...
        for my $info_Href ( @{$yLegd_Href->{info_Aref}} ){
            ...
            if( $info_Href->{shape} eq 'p' ){
                ...
                draw_a_triangle( svg_obj => $svg_obj,
                                 x => $LegIcon_X,
                                 y => $LegIcon_Y,
                                 fill_color => $info_Href->{fill_col},
