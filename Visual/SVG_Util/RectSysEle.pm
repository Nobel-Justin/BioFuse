package BioFuse::Visual::SVG_Util::RectSysEle;

use strict;
use warnings;
use Math::Trig;
use List::Util qw/ min max sum /;
use BioFuse::Visual::SVG_Util::RadSys qw/ $deg2rad /;
...
sub draw_a_parallelogram{
    ...
    # Defaults
    ## centre location
    my $x               = defined($parm{x}) ? $parm{x} : 100;
    my $y               = defined($parm{y}) ? $parm{y} : 100;
    ## segment structure
    my $hd_bot_side_len = $parm{head_bottom_side_len} || 50;
    my $lt_rt_side_len  = $parm{left_right_side_len} || $hd_bot_side_len;
    my $upleft_angle    = $parm{upleft_angle} || 90;
    ## segment appearance
    my $fill_col        = $parm{fill_color} || 'none';
    my $opacity         = (defined $parm{opacity}) ? $parm{opacity} : 1;
    my $boud_color      = $parm{boundary_color} || 'black';
    my $boud_width      = $parm{boundary_width} || 0.5;
    my $boud_dasharray  = $parm{boud_dasharray} || " ";
    my $bold_side       = $parm{bold_side} || '0';
    my $bold_width      = $parm{bold_width} || 3;
    my $draw_bool       = (defined($parm{draw_bool})) ? $parm{draw_bool} : 1;
    ## inner mark line
    my $inner_line_ori  = $parm{inner_line_ori}; # 'h' or 'v' or 'h,v'
    my $inner_line_wid  = $parm{inner_line_wid} || 1;
    my $inner_line_col  = $parm{inner_line_col} || 'black';
    ## text
    my $text_fill       = $parm{text_fill} || '';
    my $text_col        = $parm{text_col} || 'black';
    my $text_font_size  = $parm{text_font_size} || 12;
    my $text_font_fam   = $parm{text_font_fam} || 'Times New Roman';
    my $fz_auto_adjust  = $parm{font_size_auto_adjust} || 0;
    my $fz_auto_adjust_wRatio  = $parm{font_size_auto_adjust_wRatio} || 1;
    my $fz_auto_adjust_hRatio  = $parm{font_size_auto_adjust_hRatio} || 1;
    my $text_free_rotate = $parm{text_free_rotate} || 0;
    ## rotate
    my $rotate_degree   = $parm{rotate_degree} || 0;
    my $rotate_center_x = $parm{rotate_center_x} || $x;
    my $rotate_center_y = $parm{rotate_center_y} || $y;
    # Gradient color fill
    my $colorGradOrit = $parm{colorGradOrit}; # h:horizontal; v:vertical
    my $colorGradStColor = $parm{colorGradStColor} || 'red';
    my $colorGradStOpacity = $parm{colorGradStOpacity} || 0;
    my $colorGradEdColor = $parm{colorGradEdColor} || 'red';
    my $colorGradEdOpacity = $parm{colorGradEdOpacity} || 1;
...
sub draw_a_triangle{
    ...
    my $y               = defined($parm{y}) ? $parm{y} : 100;
    ## segment structure
    my $bot_side_len    = $parm{bottom_side_len} || 50;
    my $vertex_angle    = $parm{vertex_angle} || 60;
    ## segment appearance
    my $fill_col        = $parm{fill_color} || 'none';
    my $opacity         = (defined $parm{opacity}) ? $parm{opacity} : 1;
    my $boud_color      = $parm{boundary_color} || 'black';
...
sub draw_a_ellipse{
    ...
    my $radius          = $parm{radius} || 50;
    my $radius_b        = $parm{radius_b} || $parm{radius};
    ## segment appearance
    my $fill_col        = $parm{fill_color} || 'none';
    my $opacity         = (defined $parm{opacity}) ? $parm{opacity} : 1;
    my $boud_color      = $parm{boundary_color} || 'black';
    ...
    my $fz_auto_adjust  = $parm{font_size_auto_adjust} || 0;
    ## rotate
    my $text_rotate_degree   = $parm{rotate_degree} || 0;
    my $text_rotate_center_x = $parm{rotate_center_x} || $cx;
    my $text_rotate_center_y = $parm{rotate_center_y} || $cy;
