package BioFuse::Visual::SVG_Util::Font;

use strict;
use warnings;
use Math::Trig;
use List::Util qw/ min max sum /;
use BioFuse::Visual::SVG_Util::RadSys qw/ $deg2rad get_coordinate_on_circle draw_an_arc /;
require Exporter;
...
sub confirm_transform_ratio{
    ...
    if( scalar(@_) == 0 || $parm{usage_print} ){
...
sub show_text_in_line{
    ...
    # Defaults
    my $text_x          = defined($parm{text_x}) ? $parm{text_x} : 100;
    my $text_y          = defined($parm{text_y}) ? $parm{text_y} : 100;
    my $text            = defined($parm{text}) ? $parm{text} : 'test';
    my $font_family     = $parm{font_family} || 'Arial';
    my $font_size       = $parm{font_size} || 12;
    my $text_anchor     = $parm{text_anchor} || 'middle';
    my $text_col        = $parm{text_col} || 'black';
    my $height_adjust   = $parm{height_adjust} || 0;
    my $height_limit    = $parm{height_limit} || 0;
    my $width_limit     = $parm{width_limit} || 0;
    my $width_limit_fontsizeTorotate = $parm{width_limit_fontsizeTorotate};
    my $rotate_degree   = $parm{rotate_degree} || 0;
    my $rotate_center_x = $parm{rotate_center_x} || $text_x;
    my $rotate_center_y = $parm{rotate_center_y} || $text_y;
    my $features_Href   = defined($parm{features}) ? $parm{features} : {};
...
sub show_text_on_arc{
    ...
    my $anchor_degree      = $parm{anchor_degree} || 0;
    my $text_anchor        = $parm{text_anchor} || 'middle';
    my $text               = defined($parm{text}) ? $parm{text} : 'test';
    my $font_family        = $parm{font_family} || 'Arial';
    my $font_size          = $parm{font_size} || 12;
    ...
    my $height_adjust      = $parm{height_adjust} || 0;
    my $height_limit       = $parm{height_limit} || 0;
    my $degree_limit       = $parm{degree_limit} || 0;
...
sub get_size_of_text_to_show{
    ...
    if($parm{usage_print} || !defined($parm{text})){
        warn "\nThe Usage of $sub_routine_name:\n";
        warn "$Usage";
        warn "\n";
        exit(1);
    ...
    my $font_size        = $parm{font_size} || 12;
    my $text             = $parm{text};
    my $single_char_gap_bool = $parm{single_char_gap} || 0;
    my $return_HeightWid = $parm{return_HeightWid} || 0;
